            ...
            <div class="flex flex-col space-y-16 xl:w-5/6">
                @forelse($articles as $article)
                    <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
                        <img src="{{\Illuminate\Support\Facades\Storage::url($article->thumbnail)}}" class="border col-span-1 object-cover bg-center h-40 w-full rounded" loading="lazy" />
                        <div class="col-span-1 md:col-span-3">
                            <p class="text-sm text-gray-500 font-normal mb-2 -mt-1">{{$article->created_at->format('F j, yy')}}</p>
                            <h2 class="text-gray-800 text-xl font-extrabold leading-snug mb-2">
                                <a href="{{route('articles.show', $article->id)}}" class="text-gray-900 hover:text-orange-500">{{$article->name}}</a>
                            </h2>
                            <p class="text-sm text-gray-600 font-normal mb-3">
                                {{$article->description}}
                            </p>
                            <a href="{{route('articles.show', $article->id)}}" class="text-sm px-5 py-2 rounded bg-orange-100 text-orange-500 focus:shadow-outline-orange transition duration-100">Read More</a>
                        </div>
                    </div>
                @empty
