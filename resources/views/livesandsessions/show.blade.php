...
@section('title', $livesandsession->name)

@push('meta')
    <meta property="og:url"           content="{{url()->current()}}" />
    <meta property="og:type"          content="website" />
    <meta property="og:title"         content="{{$livesandsession->name}}" />
    <meta property="og:description"   content="{{$livesandsession->description}}" />
    <meta property="og:image"         content="{{\Illuminate\Support\Facades\Storage::url($livesandsession->thumbnail)}}" />
@endpush
...
@section('content')
    <section class="container mx-auto py-24 px-4">
        <div class="border-b border-gray-200 pb-8 mb-12">
            <h2 class="text-2xl md:text-4xl text-gray-900 leading-tight font-extrabold truncate">{{$livesandsession->name}}</h2>
        </div>
        <div class="w-full">
            <div class="flex flex-col space-y-16 xl:w-5/6">
                @forelse($livesandsession->videos as $video)
