...
@section('content')
    <section class="container mx-auto py-24 px-4">
        <div class="flex flex-wrap justify-between items-center gap-5 border-b border-gray-200 pb-8 mb-12">
            <h2 class="text-2xl md:text-4xl text-gray-900 leading-tight font-extrabold">All Playlists</h2>
            ...
            <div class="flex flex-col space-y-16 xl:w-5/6">
                @forelse($playlists as $playlist)
                    <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
                        <img src="{{\Illuminate\Support\Facades\Storage::url($playlist->thumbnail)}}" class="border col-span-1 object-cover bg-center h-40 w-full rounded" loading="lazy" />
                        <div class="col-span-1 md:col-span-3">
                            <p class="text-sm text-gray-500 font-normal mb-2 -mt-1">{{$playlist->created_at->format('F j, yy')}}</p>
                            <h2 class="text-gray-800 text-xl font-extrabold leading-snug mb-2">
                                <a href="{{route('playlists.show', $playlist->id)}}" class="text-gray-900 hover:text-orange-500">{{$playlist->name}}</a>
                            </h2>
                            <p class="text-sm text-gray-600 font-normal mb-3">
                                {{$playlist->description}}
                            </p>
                            <a href="{{route('playlists.show', $playlist->id)}}" class="text-sm px-5 py-2 rounded bg-orange-100 text-orange-500 focus:shadow-outline-orange transition duration-100">Play</a>
                        </div>
                    </div>
                @empty
                    <p class="text-gray-600 text-center py-5">No playlists added, yet.</p>
                @endforelse
